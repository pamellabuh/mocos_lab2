...
def generate_signal_bin(filename, length):
    ...
    #массив для записи типа действ мнииманя действ мнимая и так далее
    data_to_write = np.zeros(2 * length, dtype=np.float64)
    data_to_write[0::2] = real_part    # четные- действительные части
    data_to_write[1::2] = imag_part    # нечетнще- мнимые части
